...
const calculateTaxeGroups = async (quantity, price, discountAmount, taxGroupID, currencyRate = 1, addCharges = null, totalCharges = 0, settings = null, OrignalPrice = 0, discountrate = 0, gbt) => {
  // console.log("calculateTaxeGroups helper", quantity, price, discountAmount, taxGroupID, currencyRate, addCharges, totalCharges, settings, OrignalPrice, discountrate)
  ...
  if (taxGroupID != "") {
    ...
    if (selectedproductTaxes != null) {
      ...
      taxes.Tax1Enable = selectedproductTaxes.Tax1Enable === "true" ? true : false;
      ...
      if (selectedproductTaxes.Tax1Enable === "true" && selectedproductTaxes.Tax1Code != "") {
        ...
        taxes.IsTax1IncludedInPrice = selectedproductTaxes.IsTax1InclusiveInPrice === "true" ? true : false;
          ...
          if (selectedproductTaxes.Tax1Value === 0) {
            ...
            if ((calcID > 0) && (rate > 0)) {
              ...
              if (amt > 0 && rate > 0 && selectedproductTaxes.IsTax1InclusiveInPrice === "false") {
                ...
                taxes.Tax1Amount = (settings.IsTaxOnSalesProduct === "false" && !gbt) ? 0 : Number(((amt * rate) / 100)?.toFixed(settings.DecimalsInAmount));
                ...
                if (quantity > 0) {
                  if (OrignalPrice < 1) {
                    ...
                    taxes.Tax1Amount = (settings.IsTaxOnSalesProduct === "false" && !gbt) ? 0 : Number((((((amt / quantity) * rate) / 100)) * quantity)?.toFixed(settings.DecimalsInAmount));

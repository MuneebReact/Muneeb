...
const AddSearchBuyer = ({ props})  => {
  ...
  useEffect(() => {
    ...
    const currentDate = `${year}${month < 10 ? '0' + month : month + 1}${date < 10 ? '0' + date : date
      }`;
  ...
  const addSaveBuyer = async (values) => {
    ...
    if (isAdd) {
      ...
      console.log(UserLogin, 'Buyer/CreateBuyer',"add buyer body", body)
      let response1 = await props.dispatch(
      ...
      if (initialValues.BuyerName !== '') {
        if (values === 'Save') {
          ...
          if (selectedLoyalty !== '' && !initialValues.LoyaltyCard) {
            ...
            console.log(' Values is...', initialValues);
        ...
        console.log("props testing complete 2",props.buyerInfo);
      }
      console.log("props testing complete 3",props.buyerInfo);
    }
    setLoading(false); 
  ...
  const searchBuyer = async (BCode, type) => {
    ...
    if (!props.buyerInfo || type === 'add') {
      ...
      // console.log('decodeUpadedToken', decodeUpadedToken);
      let body = {
        BuyerName: '',
        ReturnCode: 0,
        BuyerCode: null,
      ...
      let response1 = await props.dispatch(
       ...
       response1 = JSON.parse(response1);
      if (response1.ReturnCode === 1) {
        alert('There is no buyer found');
      } else {
        if (response1.ReturnCode === 0) {
          setInitialValues({
            BuyerAddress: response1.BuyerAddress,
            BuyerCode: response1.BuyerCode,
            BuyerName: response1.BuyerName,
            CCRNumber: response1.CCRNumber,
            PrimaryPhone: response1.PrimaryPhone,
            ValueAddedTaxNumber: response1.ValueAddedTaxNumber,
            CultureCode:'en-US',
          });
          if (response1.LoyaltyCard !== null) {
            if (
              ...
              const currentDate = year + '/' + month + '/' + date;
              setLoyaltyDate(currentDate);
              console.log(response1.LoyaltyCard.RegistrationDate,"pakistan testing buyer 2")
            } else {
              setLoyaltyDate('');
            }
